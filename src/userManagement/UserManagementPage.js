...
export const UserManagementPage = () => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        const response = await axios.get(
          "https://predictivemaintanance-1.onrender.com/api/users"
        );
  ...
  const handleCreateUser = async (newUser) => {
    ...
    try {
      const checkResponse = await fetch(
        "https://predictivemaintanance-1.onrender.com/check-email",
        {
          method: "POST",
    ...
    try {
      const response = await fetch(
        "https://predictivemaintanance-1.onrender.com/register",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `https://predictivemaintanance-1.onrender.com/api/users/${selectedUser._id}`
      );
      const response = await axios.get(
        "https://predictivemaintanance-1.onrender.com/api/users"
      );
  ...
  const handleUpdateUser = async (updatedUser) => {
    ...
    try {
      await axios.put(
        `https://predictivemaintanance-1.onrender.com/api/users/${updatedUser._id}`,
        updatedUser
      );
      const response = await axios.get(
        "https://predictivemaintanance-1.onrender.com/api/users"
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const response = await fetch(
        "https://predictivemaintanance-1.onrender.com/api/send-email",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
