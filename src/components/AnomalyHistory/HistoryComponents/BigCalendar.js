  ...
  const components = {
    event: (props) => {
      ...
      if (view != "month") {
        if (data.x > 32) {
          return (
            <div style={{ background: "red", color: "white" }}>{data?.x}</div>
          );
        } else if (data.x > 30) {
          return (
          ...
          return (
            <div style={{ background: "green", color: "white" }}>{data?.x}</div>
          );
        }
      } else {
  ...
  const updateEventsForView = async (newView, startDate, endDate) => {
    try {
      ...
      if (weekEvents.message && newView === "month") {
        const filtered = weekEvents.message.filter(
          (data) => data.value > 28 && data.value <= 30
        );
        const filteredCrit = weekEvents.message.filter(
          (data) => data.value > 30
        );
      ...
      if (newView === "month") {
        if (
          Array.isArray(eventsArrayReduced) &&
          eventsArrayReduced.length > 0
        ) {
